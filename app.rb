...
form = ""
  ...
  form +="<p><label>Languages to be Translated</label><select name='hasLanguage'><option value='#{thisEdit.hasLanguage}>#{thisEdit.hasLanguage}</option><option value='Arabic'>Arabic</option><option value='Russian'>Russian</option><option value='Both'>Both</option><option value='None'>None</option></select></p>"
...
get '/displaysearch' do
  ...
  if params[:hasLanguage] != "none"
    ...
    @cans.each do |can|
      output +="<p><a href='display_record/#{can.id}'>&#62 Can #{can.canNum}</a>&#32(#{can.canType})"
      output +="<img src='#{this.imageURL}' width='100' height='75'/></p>"
    end
