...
get '/display_record/:id' do
  ...
  output +='<a href="/about">&#62About</a>'
  output +='<a href="/contact">&#62Contact</a>'
  output +='<a style="text-align:right;position:relative;left:750" href="/display_record/#{params[:id]+1}">&#62Next</a>'
  output +='</div>'
...
form = ""
  ...
  form +="<p><label>Languages to be Translated</label><select name='hasLanguage'><option selected='true' value=#{thisEdit.hasLanguages}><#{thisEdit.hasLanguages}</option><option value='Arabic'>Arabic</option><option value='Russian'>Russian</option><option value='Both'>Both</option></select></p>"
